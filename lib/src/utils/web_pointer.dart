...
int pinBufferByFakeAddress(Uint8List buffer) {
  js_util.setProperty(html.window, 'pdf_render_scroll_buffer_$_fakeAddress', buffer);
  return _fakeAddress++;
}

/// Get the associated buffer for the address.
Uint8List getBufferByFakeAddress(int address) {
  return js_util.getProperty(html.window, 'pdf_render_scroll_buffer_$address')
      as Uint8List;
}

/// Release the associated buffer for the address.
void unpinBufferByFakeAddress(int address) {
  js_util.setProperty(html.window, 'pdf_render_scroll_buffer_$address', null);
}
