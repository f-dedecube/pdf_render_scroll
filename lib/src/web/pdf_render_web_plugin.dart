...
class PdfRenderWebPlugin {
  static void registerWith(Registrar registrar) {
    final channel =
        MethodChannel('pdf_render_scroll', const StandardMethodCodec(), registrar);
  ...
  final _eventChannel =
      const PluginEventChannel('jp.espresso3389.pdf_render_scroll/web_texture_events');
  ...
  void _releaseTex(int id) {
    ...
    _textures.remove(id);
    js_util.setProperty(html.window, 'pdf_render_scroll_texture_$id', null);
  }
  ...
  Future<int> _updateTex(dynamic args) async {
    return await _renderRaw(
      ...
      handleRawData: (
        ...
        _textures[id]?.dispose();
        _textures[id] = image;
        js_util.setProperty(html.window, 'pdf_render_scroll_texture_$id', image);
