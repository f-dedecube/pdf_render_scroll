...
import 'package:pdf_render_scroll/src/utils/custom_interactive_viewer.dart';
...
class PdfViewerController extends TransformationController {
  ...
  double? getTotalPageSize(int pageNumber) {
    var pages = _state?._pages ?? [];
    if (pages.isEmpty) {
      return null;
    }
...
class PdfViewerParams {
  ...
  /// Used to adjust the scroll granularity.
  /// Specifically, on Flutter Web there is some page loading jank that this
  /// attempts to smooth out.
  final double scrollOffsetRatio;

  /// Used to adjust page memory retention.
  /// The higher the value, the worse for performance
  /// Use with caution.
  final double fullPurgeDistThreshold;

  /// Used to adjust page memory retention.
  /// The higher the value, the worse for performance
  /// Use with caution.
  final double partialRemovalDistThreshold;

  /// On relayout, text can become pixelated.
  /// Use this to adjust (Was 100ms by default, now 20ms)
  final int realSizeOverlayUpdateBufferDuration;
  ...
  PdfViewerParams copyWith({
    ...
    OnPdfViewerControllerInitialized? onViewerControllerInitialized,
      ...
      PdfViewerParams(
        ...
        scrollOffsetRatio: scrollOffsetRatio ?? this.scrollOffsetRatio,
  ...
  bool operator ==(Object other) {
    ...
    return other is PdfViewerParams &&
        ...
        other.scrollOffsetRatio == scrollOffsetRatio &&
...
class PdfViewerState extends State<PdfViewer>
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomInteractiveViewer(
          ...
          constrained: false,
          scrollOffsetRatio: widget.params?.scrollOffsetRatio ?? 1.0,
  ...
  void _needRealSizeOverlayUpdate() {
    ...
    _realSizeUpdateTimer = Timer(
      Duration(
          milliseconds:
              widget.params?.realSizeOverlayUpdateBufferDuration ?? 100),
      () => _updateRealSizeOverlay(),
    );
  }
  ...
  Future<void> _updateRealSizeOverlay() async {
    ...
    for (final page in _pages!) {
      ...
      if (part.isEmpty) {
        ...
        if (dist > distBase * (widget.params?.fullPurgeDistThreshold ?? 33)) {
          page.releaseTextures();
        } else if (dist >
            distBase * (widget.params?.partialRemovalDistThreshold ?? 8)) {
